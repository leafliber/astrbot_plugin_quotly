...
复刻自 Telegram 著名的 [quote-bot](https://github.com/quote-bot)，为 AstrBot 带来同样的优雅体验
...
Quotly 是一款专为 AstrBot 设计的语录图片生成插件，可以将群聊中的精彩对话瞬间定格为精美的分享图片。
...
本插件依赖 **Playwright** 浏览器渲染引擎，安装插件后需要手动执行：

```bash
playwright install chromium
```

> 如果 AstrBot 运行在 Docker 容器中，请确保容器内已安装 Chromium 浏览器。
